...
We say $F:\R\rightarrow \R$ is a lifting of $f$ if $\pi F = f \pi$ where $\pi: \R \rightarrow S^1$ is the covering map. It’s immediate that $F(x+1)=F(x)+1$ (moreover if $G:=F+k$ then $G^n=F^n+nk$) and $F_1(x)\equiv F_2(x) (mod 1)$. The first observation indicates that we should regard a orientaion-preserving homeomorphism $f$ on $S^1$ as an unique periodic function $F(x)-x$ up to adding a constant integer. Observe that $F^k(x+1)-(x+1)=F(F^{k-1}(x)+1)-(x+1)=F^k(x)-x$ which is again perodic.

We can now define the rotation number of $F$ by $\rho(F)=\lim\limits_{n\rightarrow \infin}\frac{F^n(x)}{n}=\lim\limits_{n\rightarrow \infin}\frac{F^n(x)-x}{n}=\lim\limits_{n\rightarrow \infin}\frac{F^n(x+m)-(x+m)}{n}=\lim\limits_{n\rightarrow \infin}\frac{F^n(y)-y}{n}=\lim\limits_{n\rightarrow \infin}\frac{F^n(y)}{n}$ for any $x,y$ where $y$ must get squeezed in some interval $[x+m,x+m+1]$ as $F$ monotonically increase, which means the limitation doesn’t depend on the choice of $x$. By the above argument, $\rho(f):=\rho(F)(mod1)$ is well-defined.

Yet we need to show the limit does exist. We seperate it into two cases:

(1) $f$ admits a periodic point, say $x_0$. In the sense of lifting, this means $F^{n_0}(x_0)=x_0+k_0$ for some $n_0\in\N,k_0\in \Z$. Inductively, one has $F^{mn_0}(x_0)=x_0+mk_0,\forall m\in \N$ .

Claim: If $\lim\limits_{m\rightarrow \infin}\frac{a_{mk}}{mk}=a$ for fixed $k$ and $a_m$ bounded, then $\lim\limits_{m\rightarrow \infin}\frac{a_{m}}{m}=a$.
...
Theorem: (1) $\rho(F)=\lim\limits_{n\rightarrow \infin}\frac{F^n(x)-x}{n}$ exists and dosen’t depend on $x\in\R$ (2) $\rho(F+k)=\rho(F)+k,\forall x\in\Z$ hence $\rho(f):=\rho(F)$ is well-defined.
...
Proof: Suppose $G(x)-x>0\$ for all $x>x_0$. Since it’s continuous and periodic hence we can choose $\delta=\inf\limits_{x\in\R}G(x)-x>0$. Then $G(x)-x\geq\delta,\forall x\in \R$. By cliché trick we accumulate the error $G^k(x)-x\geq k\delta$. But this we deviate the rotation number $\rho(g)=\lim\limits_{k\rightarrow \infin}\frac{G^k(x)-x}{k}(mod1)=\delta(mod1)>0$ contradicated.
...
Then we may extend $H$ on $\R$ by setting $\hat H(x):=\begin{cases}H(x),x\in eo(x_0)\\ \sup\limits_{y\in eo(x_0),y<x}H(y)\end{cases}$.
...
Theorem: Given $\alpha\in \R\backslash \mathbb{Q}$, there exists a $C^1$-orientation-perserving diffeomorphism $f:S^1\rightarrow S^1$ with $\rho(f)=\alpha(mod1)$ such that $f$ has no dense trajectory i.e. non-transitive. In particular, $f$ is not conjugate to $R_\alpha$.
...
Key observation: $f(0)=a_1$ and $f(a_1)=a_1+\int_0^{a_1}g(t)dt=a_1+\int_{\bigcup \limits_{I_m\sub [0,a_1]}I_m}g(t)dt=a_1+\sum\limits_{I_m\sub [0,a_1]}\int_{I_m}g(t)dt=a_1+\sum\limits_{I_m\sub [0,a_1]}l_{m+1}$. Notice $I_m\sub [0,a_1]$ if and only if $m\in J_1$. Therefore, $f(a_1)=a_1+\sum\limits_{m\in J_1}l_{m+1}=a_1+\sum\limits_{k\in \Z:R^k_\alpha(x_0)\in [R_\alpha(x_0),R^2_\alpha(x_0))}l_{k}=a_1+\sum\limits_{m\in J_2\backslash J_1}l_{m}=a_2$. Inductively, we have $f^m(0)=a_m$.

Notice $\omega(0)\sub S^1\backslash \bigcup \limits_{n\in\Z}\dot I_n$, the converse is also true since $\bigcup\limits_{m\in \Z}I_m$ is dense. Since $\rho(f)$ irrational $\omega(x)=\omega(0)=S^1\backslash \bigcup \limits_{n\in\Z}\dot I_n,\forall x\in S^1$. This show that $x$ has no dense orbit indicating that $f$ is non-transitive.
...
Definition: (1)The $\omega$-limit set for $(X,f)$ of $x$ is defined by $\omega(x):=\{y\in X: \exist n_i\rightarrow +\infin s.t.f^{n_i}\rightarrow y ,i\rightarrow +\infin\}$ (2)The $\alpha$-limit set for $(X,f)$ of $x$ is defined by $\alpha(x):=\{y\in X: \exist n_i\rightarrow -\infin s.t.f^{n_i}\rightarrow y ,i\rightarrow +\infin\}$

Proposition: If $\rho(f)$ irrational, then $\omega(x)=\omega(y),\forall x,y\in S^1$.

The proposition basically based on the following observation: Given $x,z\in S^1$ and $m>n\in\N$. Then there is $k\in\N^*$s.t. $f^k(z)\in [f^n(x),f^m(x)]$ when $\rho (f)$ irrational.

Assuming this observation, we proof the proposition. For $x_0\in \omega$ i.e. $n_i\rightarrow +\infin s.t.f^{n_i}(x)\rightarrow x_0,i\rightarrow +\infin$, there is $k_i\in \N$  s.t.$f^{k_i}(y)\in [f^{n_i}(x),f^{n_{i+1}}(x)],\forall i$. One can always choose $k_{i+1}>k_i$,by plugging $x=f^{k_i}(y)$ so that $k_i\rightarrow +\infin$ as $i\rightarrow +\infin$. Since $f^{n_i}(x)\rightarrow x_0$ we have $f^{k_i}(y)\rightarrow x_0$ i.e. $x_0\in \omega (y)$ hence $\omega(x)\sub \omega(y)$. By symmetry $\omega(x)=\omega(y)$.

Now let’s prove the observation: Denote $I=[f^n(x),f^m(x)]$. Consider $\mathscr{I}:=\{\rho^{-k(m-n)}(I): k\in\N^*\}$. Notice these bunch of intervals share at most a boundary point in common. Then either(1) $\mathscr{I}$ converges to a point $y\in S^1$ or (2) a finite union of the intervals cover $S^1$ since $f$ is an orientation-preserving homeomorphism. But (1) cannot happen since $f^{-(m-n)}(y)=y$ indicates that $y$ is a periodic point leading $\rho(f)$ rational, contradicted the pre-requirement that $\rho (f)$ irrational. On the other hand, (2) establishes the observation.
